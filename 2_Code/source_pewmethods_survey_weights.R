...
### Source: The pewmethods package developed by the Pew Research Center
...
get_totals <- function(var, df, wt = NULL, by = NULL, by_total = FALSE, percent = TRUE, include_unw = FALSE, digits = NULL, complete = TRUE, na.rm = FALSE) {
  ...
  make_weighted_crosstab <- function(var, df_in, wt_var) {
    ...
    # Else if na.rm = FALSE, make NA an explicit factor level
      ...
      if (exists("var_split")) {
        df_in <- df_in %>%
          mutate_at(.vars = var_split, fct_na_value_to_level) ### -> fct_na_value_to_level/fct_explicit_na
      }
      
      else {
        df_in <- df_in %>%
          mutate_at(.vars = var, fct_na_value_to_level) ### -> fct_na_value_to_level/fct_explicit_na
      }
    }
